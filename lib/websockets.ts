...
// Extend the global object to include our custom methods
declare global {
  var emitPriceUpdate: (productId: string, newPrice: number, oldPrice: number) => Promise<void>;
  var emitAlertTriggered: (alertId: string, productId: string, userId: string) => Promise<void>;
}

// Track active connections by user ID
const userConnections: Record<string, UserConnection[]> = {};
...
function setupEventHandlers(socket: any, io: SocketIOServer) {
  const userId = socket.data.userId;
  ...
  socket.on('set:markup', async (data: { productId: string, markupPercentage: number }) => {
    try {
      const { productId, markupPercentage } = data;
      
      // Use raw SQL query as a workaround until Prisma client is properly updated
      // Format an SQL statement that works for PostgreSQL
      const query = `
...
function setupSystemEvents(io: SocketIOServer) {
  // Price update event handler for cron job to call
  global.emitPriceUpdate = async (productId: string, newPrice: number, oldPrice: number) => {
    try {
      const product = await prisma.product.findUnique({
        where: { id: productId },
        select: { userId: true }
  ...
  global.emitAlertTriggered = async (alertId: string, productId: string, userId: string) => {
    try {
